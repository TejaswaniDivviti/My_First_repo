# Perform Mo volte call after enabling usb tethering mode
import time
...
class perform_call_tethering():
    ...
    def execute_call_tethering(self):
        ...
        # performing MO call
        log.info("performing MO volte call")
        status, msg = ad.perform_a2b_call(self.Data["serialId"], self.Data['testcase_config'][self.tst]['tempDevicesId'][0],
                                            self.Data['testcase_config'][self.tst]['callA_no'],
                                            self.Data['testcase_config'][self.tst]['callB_no'])
        
        
        if not status:
            log.info("a2b mo call failed")
            return status, msg
        log.info(msg)
        time.sleep(5)
