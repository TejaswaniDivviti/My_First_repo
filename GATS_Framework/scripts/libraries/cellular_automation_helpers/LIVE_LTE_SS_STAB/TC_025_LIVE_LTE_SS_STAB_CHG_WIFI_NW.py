...
class perform_change_wifi_network:
    def __init__(self, tst, yamlData):
        self.tst = tst
        self.Data = yamlData

    def execute_wifi_network(self):
        # connecting to wifi.
        ...
        log.info("changing wifi network")
        status, msg = ad.change_wifi_connection(self.Data['serialId'],
                                                self.Data['testcase_config'][self.tst]['wifi_ssid_new'],
                                                self.Data['testcase_config'][self.tst]['wifi_password_new'])
        if not status:
            log.info("changing wifi network failed")
            return False, msg
        log.info(msg)
...
def SS_3(tst, yamlData):
    ...
    if noOfDevices >= 1:
        ...
        wifi_obj = perform_change_wifi_network(tst, yamlData)
        status, msg = wifi_obj.execute_wifi_network()
