# Perform MT volte call after enabling usb tethering mode
import time
...
class perform_call_tethering():
    ...
    def execute_call_tethering(self):
        ...
        # status, msg = ui.checking_status_tethering(self.Data['serialId'])
        # if not status:
        #     log.info("Not enabled")
        #     return status, msg

        # performing Mt call
        log.info("perform mt volte call")
        status, msg = ad.perform_a2b_call(self.Data['testcase_config'][self.tst]['tempDevicesId'][0], self.Data["serialId"],
                                            self.Data['testcase_config'][self.tst]['callB_no'], 
                                            self.Data['testcase_config'][self.tst]['callA_no'])
        if not status:
            log.info("perform mt volte call failed")
            return status, msg
        log.info(msg)
        time.sleep(5)
