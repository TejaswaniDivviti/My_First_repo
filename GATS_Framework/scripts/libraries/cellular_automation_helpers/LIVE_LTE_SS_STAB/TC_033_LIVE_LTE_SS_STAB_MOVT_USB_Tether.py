# Perform Mo vt call after enabling usb tethering mode
import time
...
class perform_call_tethering():
    ...
    def execute_call_tethering(self):
        ...
        webbrowser.open(self.Data['testcase_config'][self.tst]['url'])
        log.debug("Browser is Opening..")

        # performing MO cal
        log.info("performing MO vt cal")
        status, msg = ad.perform_a2b_vt_call(self.Data['serialId'], self.Data['testcase_config'][self.tst]['tempDevicesId'][0], self.Data['testcase_config'][self.tst]['callA_no'], self.Data['testcase_config'][self.tst]['callB_no'])
        if not status:
            log.info('failed to perform mo vt call')
            return False, msg
        log.info(msg)
